...
def finalize(thresh):
    ...
    '''Distance navigation code:
Use pre processing and obtain image from canny edge filter
'''
...
def cannyedge(frame,valL,valH,iterations,LowT,HighT):
    '''Canny edge filter, erosion and dilation filter can
         be used to filter and clear image, Highly effective in clearing image
         '''
...
while ret:
    ...
    try:
        ...
        cfw,navig_image1_canny=distance_navigation(navig_image_canny,cxc,cyc,amx)
         #Distance navigation based on water trench
    ...
    try:
        ...
        cfd,navig_image1=distance_navigation(navig_image,cx,cy,amax)
    ...
    try:
        if(cfd>cfw):
            ...
            cv2.imshow('Navigation',navig_image1_canny)
    except:
        pass
    if k==ord('w'):
        cntt=cntt+1
