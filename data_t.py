...
while (count<100):
    ...
    #print(len(data))
    if(len(data)==30):  #change it to 30 for madhukumar code
...
df = pd.DataFrame(Sensor_data, columns= ['Soil Temperature', 'MQ-4','Humidity %','H2 concentration','CO concentration','Rock distance','MQ-4 Atmosphere'])
df.to_csv (r'E:\SAHIL\RW\urc19\Task wise approach\data_transfer\all_sensor_data_transfer\sensor_data.csv', index = None, header=True)
print('Task Done')
